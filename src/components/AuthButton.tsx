...
const AuthButton = ({ children, credential }: AuthButtonProps) => {
    ...
    return (
        <button
            className="w-full flex justify-center items-center bg-secondary dark:bg-dark_secondary bg-opacity-50 text-white dark:text-black py-2 mt-5 md:mt-6 text-[1.15rem] rounded-xl"
            type="button"
            onClick={handleClick}
        >
            {children}
